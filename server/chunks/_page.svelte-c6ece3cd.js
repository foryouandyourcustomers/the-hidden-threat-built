...
import { g as require_root, J as require_baseGetTag, p as requireIsObjectLike, c as requireIsObject, x as createMachine, H as interpret, y as assign, K as not, G as GameState, L as and, B as sharedGuards, I as isEqual, M as getUser, N as getPlayer, O as isActionEventOf, D as getPlayerSide, E as isDefenderId } from './xstate.esm-a7479009.js';
...
const Square = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let $isPlacing, $$unsubscribe_isPlacing;
  ...
  $$unsubscribe_players = subscribe(players, (value) => $players = value);
  const isMoving = useSelector(machine2.service, (state) => state.matches("Playing.Gameloop.Playing.Moving"));
  $$unsubscribe_isMoving = subscribe(isMoving, (value) => $isMoving = value);
  const isPlacing = useSelector(machine2.service, (state) => state.matches("Playing.Gameloop.Playing.Placing"));
  $$unsubscribe_isPlacing = subscribe(isPlacing, (value) => $isPlacing = value);
  ...
  return `<div class="${[
    ...
    ($isMoving && $isPossibleMove || $isPlacing && $isPossiblePlacement ? "possible-move" : "") + " " + ($isMoving && !$isPossibleMove || $isPlacing && !$isPossiblePlacement ? "impossible-move" : "") + " " + ($isMoving && $isCurrentPosition ? "current-position" : "")
  ...
  })} ${$canMove && $isPossibleMove ? `<button class="move-button unstyled svelte-py9bsy" data-svelte-h="svelte-1xw0taw"><span class="svelte-py9bsy">Move</span></button>` : ``} ${$canPlace && $isPossiblePlacement ? `<button class="move-button unstyled svelte-py9bsy" data-svelte-h="svelte-4t2lbw"><span class="svelte-py9bsy">Place</span></button>` : ``} </div>`;
});
const css$4 = {
...
//# sourceMappingURL=_page.svelte-c6ece3cd.js.map
