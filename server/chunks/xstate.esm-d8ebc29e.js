...
class GameState {
  ...
  isValidPlacement(coordinate) {
    ...
    let allValidCoordinates = BOARD_SUPPLY_CHAINS.flat().filter((stage) => stage.id === stageId).map((stage) => stage.coordinate).filter((stageCoordinate) => {
      for (const [playerId, position] of objectEntries(this.playerPositions)) {
