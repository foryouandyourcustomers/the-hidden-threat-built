...
import { R as ROW_COUNT, C as COLUMN_COUNT, T as TOTAL_ROUNDS, F as FACES, i as isItemIdOfSide, b as CHARACTERS } from './constants-f9f2351d.js';
...
const css$2 = {
  code: ".rounds.svelte-fhxg8e.svelte-fhxg8e{align-items:flex-end;display:flex;flex-direction:column-reverse;gap:.5rem;width:100%}.rounds.svelte-fhxg8e .round.svelte-fhxg8e{--_min-percent:10%;--_max-percent:65%;--_percent:calc(var(--_min-percent) + (var(--_max-percent) - var(--_min-percent))*var(--round)/12);align-items:center;background-color:#dadcdf;background:color-mix(in oklab,var(--color-blue-spielbrett) calc(10% + 55%*var(--round)/12),#fff);color:var(--color-blue-transp-10);display:flex;font-size:.875rem;font-weight:500;height:2.125rem;justify-content:flex-end;padding:.3125rem 0;width:3.125rem}@supports (color:color-mix(in lch,red,blue)) and (top:var(--f )){.rounds.svelte-fhxg8e .round.svelte-fhxg8e{background:color-mix(in oklab,var(--color-blue-spielbrett) var(--_percent),#fff)}}.rounds.svelte-fhxg8e .round span.svelte-fhxg8e{text-align:center;width:3.125rem}.rounds.svelte-fhxg8e .round.current.svelte-fhxg8e{font-size:1rem;font-weight:700;width:4.5rem}",
  map: null
};
const Status = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="rounds svelte-fhxg8e">${each(new Array(TOTAL_ROUNDS), (_, i) => {
    return `<div class="${["round svelte-fhxg8e", $currentRound === i ? "current" : ""].join(" ").trim()}"${add_styles({ "--round": i })}><span class="svelte-fhxg8e">${escape(i + 1)}</span> </div>`;
  })} </div>`;
});
