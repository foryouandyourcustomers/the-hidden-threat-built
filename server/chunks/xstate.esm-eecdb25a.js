...
class GameState {
  ...
  isDefended(position) {
    return !!this.finalizedActionEvents.filter((event) => event.action === "defend").find((event) => isEqual(event.position, position));
  }
  isAttacked(position) {
    return !!this.finalizedActionEvents.filter((event) => event.action === "attack").find((event) => isEqual(event.position, position));
  }
...
//# sourceMappingURL=xstate.esm-eecdb25a.js.map
