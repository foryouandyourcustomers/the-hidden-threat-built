import { d as isPlayerGameEvent, k as guardForGameEventType, f as gameEventRequiresReaction, m as isGameEventOf, c as getPlayerSide, n as guardForGameEventAdminAction, o as guardForGameEventAction, h as isPlayerIdOfSide, l as isActionEventOf } from './player-81938fab.js';
import { N as seededRandomGenerator, A as isEqual$1, I as getStageAt, J as getStage, T as TARGETED_ATTACKS, K as BOARD_ITEMS, o as objectEntries, L as BOARD_SUPPLY_CHAINS, G as GLOBAL_ATTACK_SCENARIOS, S as STAGES, B as findStageAt } from './index3-b25e75a1.js';
...
class GameState {
  ...
  get attackInventory() {
    ...
    this.finalizedActionEvents.filter(guardForGameEventAction("attack")).forEach((event) => {
      const boardStage = getStageAt(event.position);
      const items = this.context.targetedAttacks.map((attackIndex) => TARGETED_ATTACKS[attackIndex]).find((attack) => {
        return attack.target.supplyChainId === boardStage.supplyChainId && attack.target.stageId === boardStage.id;
  ...
  get attackedAndDefendedStages() {
    ...
    const globalAttackStatuses = this.globalAttackScenario.attacks.map(
      () => ({
        defended: [],
        missed: [],
        successful: void 0
    ...
    this.finalizedActionEvents.forEach((event, i) => {
      ...
      if ((i + 1) % (this.playersInOrder.length * 3) === 0 && this.nextEventType !== "reaction") {
        currentGlobalAttackStatus.successful = currentGlobalAttackStatus.defended.length < currentGlobalAttack.targets.length;
...
//# sourceMappingURL=game-state-29cc4d08.js.map
