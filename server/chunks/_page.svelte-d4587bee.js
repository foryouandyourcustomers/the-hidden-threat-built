...
const isTargetedAttack = (attack) => Object.hasOwn(attack, "target");
...
const css$3 = {
  code: ".scenarios.svelte-1plqfu5.svelte-1plqfu5{display:flex;flex-direction:column;height:100%}.description.svelte-1plqfu5.svelte-1plqfu5{flex:1}.attacks.svelte-1plqfu5.svelte-1plqfu5{align-items:flex-end;display:flex;gap:1rem;list-style:none;margin:0;overflow-x:scroll;padding:0;width:100%}.attacks.svelte-1plqfu5 .attack button.svelte-1plqfu5{text-wrap:nowrap;align-items:center;border-radius:var(--radius-xs);color:#fff;display:flex;flex-direction:column;flex-shrink:0;font-size:var(--scale-0000);font-weight:700;height:5.25rem;justify-content:flex-end;padding:.5rem 0;position:relative;text-align:center;width:3.625rem}.attacks.svelte-1plqfu5 .attack button.svelte-1plqfu5 svg{bottom:auto;left:.25rem;position:absolute;right:.25rem;top:.75rem}.attacks.svelte-1plqfu5 .attack button.attack.svelte-1plqfu5{background-color:var(--color-red-polygon)}.attacks.svelte-1plqfu5 .attack button.defense.svelte-1plqfu5{background-color:var(--color-orange-dark)}.attacks.svelte-1plqfu5 .attack button.selected.svelte-1plqfu5{box-shadow:0 0 8px 0 rgba(88,90,90,.4);height:5.875rem;width:4rem}.targets.svelte-1plqfu5.svelte-1plqfu5{display:flex;gap:2rem}.targets.svelte-1plqfu5 .stage.svelte-1plqfu5{font-size:.75rem}.targets.svelte-1plqfu5 .items.svelte-1plqfu5{display:flex;gap:.5rem}",
  map: null
};
const Scenarios = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let selectedAttack;
  ...
  const activeAttacks = useSelector(
    ...
    (state) => {
      const gameState = GameState.fromContext(state.context);
      const side2 = getCurrentUser(state.context).side;
      return side2 === "attack" ? gameState.getActiveTargetedAttacks() : gameState.getActiveGlobalAttacks();
    },
    isEqual
  );
  ...
  const totalAttackCount = useSelector(machine2.service, (state) => {
    const side2 = getCurrentUser(state.context).side;
    return side2 === "attack" ? state.context.targetedAttacks.length : state.context.globalAttacks.length;
  });
  ...
  return `<div class="scenarios svelte-1plqfu5"><div class="description svelte-1plqfu5">${validate_component(Paragraph, "Paragraph").$$render($$result, { spacing: "none", size: "sm" }, {}, {
    default: () => {
      return `${escape($side === "attack" ? "Gezielter" : "Allgemeiner")} Angriff`;
    }
  ...
  })} <div class="targets svelte-1plqfu5">${isTargetedAttack(selectedAttack) ? `<div class="target"><div class="items svelte-1plqfu5">${each(selectedAttack.target.requiredItems, (item) => {
    return `${validate_component(Item, "Item").$$render($$result, { itemId: item }, {}, {})}`;
  })}</div></div>` : `${each(selectedAttack.targets, (target) => {
    return `<div class="target"><div class="stage svelte-1plqfu5">${escape(getStage(target.stageId).name)}</div> <div class="items svelte-1plqfu5">${each(target.requiredItems, (item) => {
      return `${validate_component(Item, "Item").$$render($$result, { itemId: item }, {}, {})}`;
    })}</div> </div>`;
  })}`}</div></div> <ul class="attacks svelte-1plqfu5">${each(new Array($totalAttackCount), (_, index) => {
    return `<li class="attack"><button ${($side === "attack" ? index >= $activeAttacks.length : index !== selectedAttackIndex) ? "disabled" : ""} class="${[
      "unstyled " + escape($side, true) + " svelte-1plqfu5",
      selectedAttackIndex === index ? "selected" : ""
      ...
      {},
      {}
    )} <span>Angriff ${escape(index + 1)} </span></button> </li>`;
  })}</ul> </div>`;
});
const css$2 = {
...
//# sourceMappingURL=_page.svelte-d4587bee.js.map
