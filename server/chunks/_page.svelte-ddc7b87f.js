...
import { g as require_root, G as require_baseGetTag, p as requireIsObjectLike, c as requireIsObject, x as createMachine, E as interpret, y as assign, H as not, z as sharedGuards, F as isEqual, J as FACES, C as isDefenderId } from './isEqual-ac1b14e5.js';
...
const PlayerConfigurator = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const player = useSelector(machine2.service, ({ context }) => {
    return side === "attack" ? context.attack.attacker : context.defense.defenders[playerId];
  });
...
const css$4 = {
  code: '.square.svelte-166sgyr.svelte-166sgyr{display:grid;grid-column:var(--_column);grid-row:var(--_row);grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(3,1fr);position:relative}.square.svelte-166sgyr.svelte-166sgyr,.square.svelte-166sgyr>.svelte-166sgyr{min-height:0;min-width:0}.square.svelte-166sgyr.svelte-166sgyr:after{border:1px solid hsla(0,0%,100%,.133);bottom:calc((0px - var(--px))/2);content:"";left:calc((0px - var(--px))/2);pointer-events:none;position:absolute;right:calc((0px - var(--px))/2);top:calc((0px - var(--px))/2)}.player.svelte-166sgyr.svelte-166sgyr{grid-column:2;grid-row:2}.item.svelte-166sgyr.svelte-166sgyr:first-child{grid-column:1;grid-row:1}.item.svelte-166sgyr.svelte-166sgyr:nth-child(2){grid-column:3;grid-row:3}.item.svelte-166sgyr svg,.player.svelte-166sgyr svg{height:100%;width:100%}',
  map: null
};
const Square = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const players = useSelector(machine2.service, ({ context }) => [...context.defense.defenders, context.attack.attacker].filter((player) => isEqual(player.position, coordinate)), isEqual);
  $$unsubscribe_players = subscribe(players, (value) => $players = value);
  ...
  $$unsubscribe_items();
  $$unsubscribe_players();
  return `<div class="square svelte-166sgyr"${add_styles({
    "--_row": rowIndex + 1,
    "--_column": columnIndex + 1
  })}>${each($items, (item) => {
    return `<div class="item svelte-166sgyr">${validate_component(Item, "Item").$$render($$result, { itemId: item.item }, {}, {})} </div>`;
  })} ${each($players, (player) => {
    return `<div class="player svelte-166sgyr">${validate_component(Face, "Face").$$render($$result, { faceId: player.faceId }, {}, {})} </div>`;
  })} </div>`;
});
const css$3 = {
...
const css$2 = {
  code: ".player.svelte-1juz5r4{align-items:center;display:flex;flex-direction:column;gap:.5rem;position:relative}.online-status.svelte-1juz5r4{background:orange;border-radius:var(--radius-full);height:.5rem;position:absolute;right:0;top:0;width:.5rem}.online-status.connected.svelte-1juz5r4{background:green}",
  map: null
};
const Player = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="player svelte-1juz5r4">${validate_component(Face, "Face").$$render($$result, { faceId: faceId ?? 0 }, {}, {})} ${escape(name)} <div class="${["online-status svelte-1juz5r4", isConnected ? "connected" : ""].join(" ").trim()}"></div> </div>`;
});
const css$1 = {
...
//# sourceMappingURL=_page.svelte-ddc7b87f.js.map
