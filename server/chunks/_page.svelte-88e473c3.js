...
import { g as require_root, F as require_baseGetTag, p as requireIsObjectLike, c as requireIsObject, x as createMachine, D as interpret, y as assign, G as not, H as and, z as sharedGuards, E as isEqual, I as getUser, J as guardForGameEventType, B as isDefenderId } from './isEqual-0e4e10ba.js';
import { a as ITEMS, i as isDefenseItemId, b as isAttackItemId, F as FACES, C as CHARACTERS } from './constants-edad85c0.js';
import { A as Actions, P as Paragraph } from './Paragraph-91fb6104.js';
...
const getInventories = (context) => {
  ...
  const inventories = {
    ...
    attack: Object.fromEntries(attackInventoryIds.map((id) => [id, 0]))
  };
...
const TempActionButton = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const getCollectActionEvent = (itemId, context) => {
    return {
  ...
  const canPerformAction = useSelector(machine2.service, (state) => state.can(getCollectActionEvent("alarm-system", machine2.service.getSnapshot().context)));
  ...
  const itemInventories = useSelector(
    ...
    (state) => {
      ...
      return {
        attack: gameState.attack.inventory,
        defense: gameState.defense.inventory
      };
    },
    isEqual
  );
...
${each(objectEntries($itemInventories.attack), ([itemId, count]) => {
    return `${count > 0 ? `<div>${validate_component(Item, "Item").$$render($$result, { itemId }, {}, {})}: ${escape(count)}</div>` : ``}`;
  })} ${$canPerformAction ? `<button data-svelte-h="svelte-10hxpqw">Collect item</button>` : ``}`;
});
const css$6 = {
...
const css$1 = {
  code: ".playing.svelte-10gsa9z{grid-gap:1rem;display:grid;gap:1rem;grid-template-columns:1fr auto 20%;position:relative}",
  map: null
};
const Playing = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  $$result.css.add(css$1);
  return `<div class="playing svelte-10gsa9z"><div class="actions">${validate_component(Players, "Players").$$render($$result, {}, {}, {})} ${validate_component(TempActionButton, "TempActionButton").$$render($$result, {}, {}, {})}</div> <div class="board">${validate_component(Board, "Board").$$render($$result, {}, {}, {})}</div> <div class="status"></div> </div>`;
});
const css = {
...
//# sourceMappingURL=_page.svelte-88e473c3.js.map
