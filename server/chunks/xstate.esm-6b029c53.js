...
import { I as ITEMS, b as isDefenseItemId, a as isAttackItemId } from './items-ba2c7988.js';
    ...
    attacks: [
      {
        ...
        description: "Schleust minderwertige und gefälschte Materialien in die Lieferkette ein und sorgt dadurch für Produktionsausfälle sowie Kundenbeschwerden."
      },
      {
...
class GameState {
  ...
  get defendedStages() {
    return this.finalizedActionEvents.filter(guardForGameEventAction("defend")).map(
      (event) => BOARD_SUPPLY_CHAINS.flat().find((stage) => isEqual(stage.coordinate, event.position)) ?? throwIfNotFound()
    );
  }
  get attackedStages() {
    const stages = this.finalizedActionEvents.filter(guardForGameEventAction("attack")).map(
      (event) => BOARD_SUPPLY_CHAINS.flat().find((stage) => isEqual(stage.coordinate, event.position)) ?? throwIfNotFound()
    );
    ...
    chainAttackCounts.forEach((count, chainId) => {
      if (count >= 3) {
        const otherStages = BOARD_SUPPLY_CHAINS[chainId].filter((stage) => !stages.includes(stage)).filter((stage) => !this.isDefended(stage.coordinate));
...
//# sourceMappingURL=xstate.esm-6b029c53.js.map
