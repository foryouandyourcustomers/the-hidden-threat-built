import { c as create_ssr_component, a as subscribe, v as validate_component, e as escape, o as onDestroy, s as setContext, b as each, d as add_attribute, g as getContext, f as add_styles } from './ssr-3cdd5219.js';
import { r as readable, w as writable } from './index2-60faee12.js';
import { b as require_root, r as require_baseGetTag, d as requireIsObjectLike, a as requireIsObject, f as createMachine, k as interpret, g as assign, n as not, s as sharedGuards, F as FACES, j as isDefenderId } from './isObjectLike-3d6a1299.js';
...
const css$6 = {
  code: ".cursor.svelte-1y3w9l8.svelte-1y3w9l8{height:1px;width:1px}.cursor.svelte-1y3w9l8.svelte-1y3w9l8,.cursor.svelte-1y3w9l8 svg.svelte-1y3w9l8{left:0;position:absolute;top:0}.cursor.svelte-1y3w9l8 svg.svelte-1y3w9l8{height:var(--size-6);translate:-10% -10%;width:var(--size-6)}.cursor.svelte-1y3w9l8 .name.svelte-1y3w9l8{background:#000;border-radius:var(--radius-sm);display:inline-block;font-size:var(--scale-000);left:1rem;max-width:var(--size-30);overflow:hidden;padding:var(--size-1) var(--size-2);position:absolute;text-overflow:ellipsis;top:1rem;white-space:nowrap}",
  map: null
};
...
const AssigningSides = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<h1 data-svelte-h="svelte-15kivf7">Einteilung in Teams</h1> <div class="users">${each($users, (user) => {
    return `<div class="user">${escape(user.name)} ${user.isAdmin ? `<span class="admin" data-svelte-h="svelte-fku40z">admin</span>` : ``} ${user.isSideAssigned ? `<span class="side svelte-190hwhb">${user.side === "attacker" ? `Angriff` : `Verteidigung`} </span>` : ``} ${$canAssignSides ? `<button data-svelte-h="svelte-1hqe603">Angriff</button> <button data-svelte-h="svelte-1yuwb2t">Verteidigung</button>` : ``} ${$canAssignAdmin && user.id !== userId ? `<button data-svelte-h="svelte-mqfmin">Toggle Admin</button>` : ``} </div>`;
...
const css$1 = {
  code: ".players.svelte-c6773x{grid-gap:1rem;display:grid;grid-template-columns:repeat(4,1fr)}",
  map: null
};
const AssigningRoles = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const { machine: machine2 } = getGameContext();
  const side = useSelector(machine2.service, ({ context }) => {
    const { side: side2 } = getUser(context);
    return side2;
  });
  $$unsubscribe_side = subscribe(side, (value) => $side = value);
  const editingPlayerId = useSelector(machine2.service, ({ context }) => ($side === "attacker" ? context.attack : context.defense).editingPlayer);
  $$unsubscribe_editingPlayerId = subscribe(editingPlayerId, (value) => $editingPlayerId = value);
  const players = useSelector(machine2.service, ({ context }) => $side === "attacker" ? [context.attack.attacker] : context.defense.defenders);
  ...
  return `<div class="players svelte-c6773x">${each($players, (player, i) => {
    return `<div class="player">${player.isConfigured ? `${escape($users.find((user) => user.id === player.userId)?.name)}` : `Nicht konfiguriert`} <button ${!$canEdit ? "disabled" : ""}>Rolle bestimmen</button> </div>`;
  })}</div> ${$editingPlayerId !== void 0 ? ` ${validate_component(PlayerConfigurator, "PlayerConfigurator").$$render($$result, { playerId: $editingPlayerId }, {}, {})}` : ``}`;
});
...
const css = {
  code: '.game-wrapper.svelte-1sdy1r9{align-content:center;background:#000;display:grid;height:100%;justify-content:center;place-content:center;width:100%}.game.svelte-1sdy1r9{background:#1b253a;display:grid;grid-template-areas:"name characters items" "actions content content";grid-template-columns:10% 1fr 1fr;grid-template-rows:10% 1fr;height:50.625rem;overflow:hidden;position:relative;width:90rem}@media(max-width:1439.98px) or (max-height:809.98px){.game.svelte-1sdy1r9{scale:.8;transform-origin:center}}@media(max-width:1199.98px) or (max-height:674.98px){.game.svelte-1sdy1r9{scale:.6}}.name.svelte-1sdy1r9{grid-area:"name"}.characters.svelte-1sdy1r9{grid-area:"characters"}.items.svelte-1sdy1r9{grid-area:"items"}.actions.svelte-1sdy1r9{grid-area:"actions"}.content.svelte-1sdy1r9{grid-area:"content"}',
  map: null
};
const Game = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  $$unsubscribe_section();
  return `<div class="game-wrapper svelte-1sdy1r9"> <div class="game svelte-1sdy1r9"${add_attribute("this", gameContainer, 0)}><div class="name svelte-1sdy1r9" data-svelte-h="svelte-iqe0dn">The Hidden Threat</div> <div class="characters svelte-1sdy1r9"></div> <div class="items svelte-1sdy1r9">${slots.items ? slots.items({}) : ``}</div> <div class="actions svelte-1sdy1r9">${slots.actions ? slots.actions({}) : ``}</div> <div class="content svelte-1sdy1r9">${$section === "Lobby" ? `${validate_component(Lobby, "Lobby").$$render($$result, {}, {}, {})}` : `...`}</div> ${slots["cursor-overlays"] ? slots["cursor-overlays"]({}) : ``}</div> </div>`;
});
let howl;
const sprite = {
  capture: [0, 204],
