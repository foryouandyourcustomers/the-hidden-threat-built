...
import { B as Button, i as is_void } from './Button-a5ee912b.js';
...
const AssigningSides = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  Teams einteilen.</p> <div class="users svelte-1w8krv2">${each($users, (user) => {
    return `<div class="user">${escape(user.name)} ${user.isAdmin ? `<span class="admin" data-svelte-h="svelte-16777ag">(admin)</span>` : ``} ${user.isSideAssigned ? `<span class="side svelte-1w8krv2">${user.side === "attacker" ? `Angriff` : `Verteidigung`} </span>` : ``} ${$canAssignSides ? `${validate_component(Button, "Button").$$render($$result, {}, {}, {
      default: () => {
        return `Angriff`;
      }
    ...
    })}` : ``} ${$canAssignAdmin && user.id !== userId ? `${validate_component(Button, "Button").$$render($$result, {}, {}, {
      default: () => {
        return `Toggle Admin`;
      }
    })}` : ``} </div>`;
  })}</div> ${$canContinue ? `${validate_component(Button, "Button").$$render($$result, {}, {}, {
    default: () => {
      return `Next`;
    }
  })}` : ``}`;
...
const css$1 = {
  code: "section.svelte-11fuvol.svelte-11fuvol{background:var(--color-bg-secondary);border-radius:var(--radius-md);margin-top:3rem;padding:1rem 1.25rem}.players.svelte-11fuvol.svelte-11fuvol{grid-gap:1rem;display:grid;grid-template-columns:repeat(4,1fr);margin-top:1.25rem}.players.svelte-11fuvol .player.svelte-11fuvol{background:var(--color-bg);border-radius:var(--radius-md);display:grid;padding:1.25rem}",
  map: null
};
const AssigningRoles = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })} <section class="svelte-11fuvol">${validate_component(Heading, "Heading").$$render($$result, { centered: true }, {}, {
    default: () => {
      return `${escape($side === "attacker" ? "Angriff" : "Verteidigung")}`;
    }
  })} ${validate_component(Paragraph, "Paragraph").$$render($$result, {}, {}, {
    default: () => {
  ...
  })} <div class="players svelte-11fuvol">${each($players, (player, i) => {
    return `<div class="player svelte-11fuvol">${player.isConfigured ? `${validate_component(Heading, "Heading").$$render($$result, { centered: true, size: "sm" }, {}, {
      default: () => {
        return `${escape(player.role)}`;
      }
    })} ${escape($users.find((user) => user.id === player.userId)?.name)}` : ``} ${validate_component(Button, "Button").$$render($$result, { size: "small", disabled: !$canEdit }, {}, {
      default: () => {
        return `Rolle ${escape(player.isConfigured ? "wechseln" : `${i + 1} bestimmen`)} `;
      }
    })} </div>`;
  })}</div></section> ${$editingPlayerId !== void 0 ? ` ${validate_component(PlayerConfigurator, "PlayerConfigurator").$$render($$result, { playerId: $editingPlayerId }, {}, {})}` : ``}`;
});
...
//# sourceMappingURL=_page.svelte-e41e41fb.js.map
