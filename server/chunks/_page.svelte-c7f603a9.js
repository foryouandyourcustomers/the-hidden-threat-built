...
const PlayerConfigurator = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })}</select> <div class="faces svelte-u0xhbm">${each(FACES, (face) => {
    return `<button ${!$canUpdate ? "disabled" : ""} class="${["face svelte-u0xhbm", face.id === faceId ? "active" : ""].join(" ").trim()}">${validate_component(Face, "Face").$$render($$result, { faceId: face.id }, {}, {})} </button>`;
...
//# sourceMappingURL=_page.svelte-c7f603a9.js.map
